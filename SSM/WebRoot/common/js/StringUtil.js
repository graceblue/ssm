...
function toSubstr(value){
	if(value !=null&&value.length>=20){
		var content = value.substring(0,16);
...
/*
	没有查询到数据   显示为空
	对jquyer ui  myview方法扩展   依赖easyui包
	参考页面：[ userorderlist.jsp],或[task_fro_person.jsp]
	datagrid 属性   view: myview,  //  myview=myviewExtend();
	例如：
	view: myview,  //获取扩展视图
	emptyMsg: '没有查询到数据',
*/
function myviewExtend(){
...
function showScroll(){
	...
	var _wid=$(".datagrid-view2 .datagrid-header-row").width();
	$(".datagrid-body").width(_wid);
	dv2.children(".datagrid-body").html("<div style='width:"+_wid+"px;border:solid 0px;height:1px;'></div>");
}
...
(function() {
  var u="//ossanalysis.telecomjs.com:9088/monitor/"; 
  _paq.push(['setTrackerUrl', 'http://ossanalysis.telecomjs.com:9088/monitor/']); 
  _paq.push(['setSiteId', 16]);
  var d=document, g=d.createElement('script'), s=d.getElementsByTagName('script')[0];
  g.type='text/javascript'; g.async=true; g.defer=true; g.src=u+'uam_noc_red.js'; s.parentNode.insertBefore(g,s);
})();
