 ...
 * @returns
 */
function getTaskTypeDic(keystrs,areaId,table){
	var jsonData=null;
	var privilege; 
	if(areaId==2){
		privilege = 1;
	...
	$.ajax({
		url:getProjectUrl()+"/status/getTaskTypeDic.do",
		async : false,//同步
		data:{"key":keystrs,'privilege' : privilege,'table':table},
		dataType :"json",//json数据格式
		success:function(data){
			jsonData = eval(data); //简单的绑定
		}
...
function getTaskTypeDicForSsyc(keystrs,areaId,table){
	var jsonData=null;
	var privilege; 
	if(areaId==2){
		privilege = 1;
	...
	$.ajax({
		url:getProjectUrl()+"/status/getTaskTypeDicForSsyc.do",
		async : false,//同步
		data:{"key":keystrs,'privilege' : privilege,'table':table},
		dataType :"json",//json数据格式
		success:function(data){
...
function getTaskTypeDicForXczh(keystrs,areaId,table){
	var jsonData=null;
	var privilege; 
	if(areaId==2){
		privilege = 1;
	...
	$.ajax({
		url:getProjectUrl()+"/status/getTaskTypeDicForXczh.do",
		async : false,//同步
		data:{"key":keystrs,'privilege' : privilege,'table':table},
		dataType :"json",//json数据格式
		success:function(data){
...
function getDicByPrivilege(keystrs,privilege,table){
	$.ajax({
		url:getProjectUrl()+"/status/getDicByPrivilege.do",
		async : false,//同步
		data:{"key":keystrs,'privilege' : privilege,'table':table},
		dataType :"json",//json数据格式
		success:function(data){
...
$("#" + linkDivId).after("<div id='linkDiv" + linkDivId + "' style='-moz-opacity:0.9;opacity: 0.9; filter:alpha(opacity=70);text-align:left;background-color:" + options.bgcolor + ";color:" + options.ftcolor + ";width:" + options.width + ";display:none;top:" + top + "px;left:" + left + "px;position:absolute;float:left'>" + options.helpText + "</div>"); 
