...
function onRename(e, treeId, treeNode) {
	if (className != treeNode.name) {
		...
		if (result && result == "n") {
			...
			/*
			 * var name = treeNode.audititemname; treeNode.title=name;//标题显示
			 * treeNode.audititemname =
...
function getData() {
	...
	if (zTreeObj) {
		var queryParams = {
			typeId : zTreeObj.id,
			fileName : $("#serName").val(),
			flag : isStringNull(flag) ? flag : ""
		};
		rows = Tool_Ajax(curProjectUrl + "/template/queryTempFile.do", {
...
function callBackNTKO(result) {
	if (isStringNull(result)) {
		...
		if (file) {
			...
			var result = Tool_Ajax(url, {
				tempFile : JSON.stringify(tempFile)
			});
			if (result && result == "y") {
				loadTempFiles();
				idG = "";
...
function uploadTempFile(obj, sessionId) {
	if (!zTreeObj || zTreeObj.id == "0") {
		...
		// 新
		commonUploadifyFile(dealwithTempFile, sessionId, {
			closeable : true,
			multi : false
		});
	}
...
function uploadFile(obj, sessionId) {
	commonUploadifyFile(callbackUpLoad, sessionId, {
		closeable : true,
		multi : false
	});
}
	...
	$('#auditDept').tree(
			{
				...
				onBeforeExpand : function(node, param) {
					$('#auditDept').tree('options').url = curProjectUrl
...
function append(){
	...
	$.messager.prompt("单位名称","请输入单位名称：",function(r){
		if(r){
			if(r.length>50){
				myAlert("新增失败？输入名称过长，最长50。");
				return;
			}
			
			var id = Tool_Ajax(curProjectUrl + "/auditDept/save.do",{depName:r,fatherDep:node.id,depKind:"0"});
			if(id){
				t.tree('append', {
...
/**删除**/
function remove(){
	var node = $('#auditDept').tree('getSelected');
	if(!$('#auditDept').tree('isLeaf', node.target)){
		myAlert("请先删除子单位！");
		return;
	}
	var url = curProjectUrl + "/auditDept/delete.do";
	var r = Tool_Ajax(url,{aid:node.id});
	if(r=="1")
		$('#auditDept').tree('remove', node.target);
	else
		myAlert("删除审计单位失败！");
}
...
function loadData(id){
	var url = curProjectUrl + "/auditDept/queryById.do";
	var data = Tool_Ajax(url,{aid:id},"json");
	//值的设置
	for(var id in data){
		if(id == "sharesScale" || id == "regCapital"){
	...
	//设定所属单位
	$("#companyid").val($("#orgName").val());
}

function saveAuditDept(){
	if(isSaving==0){//重复标识过滤
		var node = $('#auditDept').tree('getSelected');
		if(node){
			isSaving=1;
			var url = curProjectUrl +"/auditDept/update.do";
			var r = Tool_Ajax(url,getAuditDept());
			if(r=="1"){
				myAlert("保存成功！");
				//更新节点名称 
				if (node){
					node.text = $("#depName").val();//更新单位名称 
					$('#auditDept').tree('update', node);
				}
...
/**获取页面数据**/
function  getAuditDept(){
	var auditDept={};
	$(".easyui-tabs input").each(function(){
		auditDept[this.id]=this.value;
	});
	//number类型获取真实数据
	auditDept.regCapital=$("#regCapital").numberbox("getValue");//注册资金
	auditDept.sharesScale=$("#sharesScale").numberbox("getValue");//股权比例
	return auditDept;
